...
export default function PartProgress({ gameState, chickenColor, onPackage }: Props) {
  return (
    <div className="flex flex-col gap-2.5">
      {PART_ORDER.map((partId) => {
        ...
        return (
          <div key={partId} className="flex items-center gap-2.5">
            <div className="w-16 flex items-center gap-1">
              <span className="text-sm">{part.packaged ? "📦" : PART_EMOJI[partId]}</span>
              <span
                className="text-[13px] font-bold"
                style={{ color: part.packaged ? chickenColor : part.completed ? "#FFD700" : "#4e5968" }}
              >
                {part.name}
              </span>
            </div>
              ...
              <div
                ...
                style={{
                  ...
                  background: part.packaged
                    ...
                    : `linear-gradient(90deg, ${chickenColor}60, ${chickenColor}90)`,
                }}
              />
              {percent > 8 && (
